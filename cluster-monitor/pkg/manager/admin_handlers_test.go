...
func TestInitializeAfterRestart(t *testing.T) {
	testDir := t.TempDir()
	...
	require.NoError(t, err)
	require.True(t, mgr.initialized, "Manager was not initialized")

	// In normal operation the application would be stopped and restarted here.
	// We simulate this by creating a new manager with the same configuration.
	mgr.Stop()
	err = mgr.store.Close()
	require.NoError(t, err)

	mgr, err = NewManager(cfg)
	require.NoError(t, err, "Manager errored on re-initialize after restart")
	require.True(t, mgr.initialized, "Manager was not re-initialized")
}
