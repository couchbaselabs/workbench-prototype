...
import (
	...
	cblog "github.com/couchbase/tools-common/log"
...
func (c *cblogZapLogger) Log(level cblog.Level, format string, args ...interface{}) {
	switch level {
	case cblog.LevelTrace, cblog.LevelDebug:
		c.Logger.Debugf(format, args...)
	case cblog.LevelInfo:
		c.Logger.Infof(format, args...)
	case cblog.LevelWarning:
		c.Logger.Warnf(format, args...)
	case cblog.LevelError:
		c.Logger.Errorf(format, args...)
	case cblog.LevelPanic:
...
func Init(level zapcore.Level, logDir string) error {
	...
	cblog.SetLogger(&cblogZapLogger{Logger: logger.Sugar()})
